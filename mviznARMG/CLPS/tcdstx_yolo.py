...
if os.path.exists('config/usecv2dnn'):
    ...
    usecv2dnn=0
if usecv2dnn:
    tcdsyolo=YOLO("TCDS/weights/TCDStx.weights")
    tcdsyolo.inferold(dummyimage)
    while True:
        try:
            f=min(glob.glob('/dev/shm/tcdstx_yolo/*.jpg'))
            if 1:
                print(0)
    ...
    while True:
        try:
            f=min(glob.glob('/dev/shm/tcdstxyolo/*.jpg'))
            if 1:
                print(0)
                if not waittillvalidimage(f):
                    os.unlink(f)
