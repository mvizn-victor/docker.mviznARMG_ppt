...
NOW=datetime.now()
KILLTIME=NOW+timedelta(minutes=1)
open('/opt/st_clpstcds.log','a').write(NOW.strftime('CLPS_STARTED:%Y-%m-%d_%H-%M-%S')+'\n')
...
def clpscam(camname):
    ...
    while True:
        ...
        NOW=datetime.now()
        if NOW>KILLTIME:
            open('/opt/st_clpstcds.log','a').write(NOW.strftime('CLPS_KILLED:%Y-%m-%d_%H-%M-%S')+'\n')
            return
