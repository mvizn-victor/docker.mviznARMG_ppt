...
def angle(v1,uv):
    #v1 clockwise of u:-ve
    return np.arcsin(crossprod(v1,uv)/np.linalg.norm(v1))
unitvector=dict()
for camname in camnames:
    unitvector[camname]=[0,1]
    f=f'config/hncds/{camname}.txt'
    if os.path.exists(f):
        x1,y1,_,_,_,_,x2,y2=open(f).read().strip().split()
...
if SIDE=='l':
    minthreshx = dict(txxx=-25, cnxx=-9999)
    maxthreshx = dict(txxx=9999, cnxx=9999)
elif SIDE=='s':
    minthreshx = dict(txxx=-9999, cnxx=-9999)
    maxthreshx = dict(txxx=25, cnxx=9999)
    ...
    while True:
        ...
        if lastimage[camname] is not None:
            ...
            for k2, (k1, age) in linktoorigin[camname].items():
                ...
                dpars[(x1[-1],y1[-1])].append(
                 dotprod([x2[-1] - x1[-1],y2[-1] - y1[-1]],unitvector[camname])
                )
                dperps[(x1[-1],y1[-1])].append(
                 crossprod([x2[-1] - x1[-1],y2[-1] - y1[-1]],unitvector[camname])
                )
            dup=dict()
            ddown=dict()
            dright=dict()
            ...
            for k in ds:
                dup[k]=min(ds[k])
                ddown[k]=max(ds[k])
                dright[k]=max(dpars[k])
                dleft[k]=min(dpars[k])
                dperpup[k]=max(dperps[k])
                dperpdown[k]=min(dperps[k])
