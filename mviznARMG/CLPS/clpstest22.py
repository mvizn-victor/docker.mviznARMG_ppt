...
outdir=os.path.dirname(prefix).replace('/raw',f'/flowtest22/{origcamname}')
#outdir=sys.argv[2]
...
if 1:
    for f,t in zip(fs,ts):
        ...
        if lastimage[camname] is not None:
            ...
            #print(fgmask[camname].shape,fgmask[camname])
            out = list((tuple(x[0]), tuple(x[1])) for x in out if dist(x[1], x[0]) <= distcutoff and fgmask[camname][int(x[1][1]),int(x[1][0])]>0 and fgmask[camname][int(x[0][1]),int(x[0][0])]>0 or dist(x[1],x[0])<=distcutoff_stationarydist)
            #print(out)
        ...
        image_cpy[camname][fgmask[camname]<127]//=2
