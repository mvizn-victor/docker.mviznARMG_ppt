...
outdir=os.path.dirname(prefix).replace('/raw',f'/flowtest18/{origcamname}')
#outdir=sys.argv[2]
...
if 1:
    for f,t in zip(fs,ts):
        ...
        if lastimage[camname] is not None:
            ...
            out = list((tuple(x[0]), tuple(x[1])) for x in out if dist(x[1], x[0]) <= distcutoff and fgmask[camname][int(x[1][1]),int(x[1][0])]>0 or dist(x[1],x[0])<=distcutoff_low)
            print(out)
            ...
            for x in out:
                tmp1b = withindist(x[0], tmp1, distcutoff_low)
                if len(tmp1b) > 0:
