...
#model_path='CLPS/weights/clps.h5'
model_path='/home/mvizn/Code/Mask_RCNN/samples/voc_clps_20191202/logs/resnet50_sideonly.h5'
...
VocConfig.BACKBONE="resnet50"
InferenceConfig.BACKBONE="resnet50"
...
targetdir,frame,borf=sys.argv[1],int(sys.argv[2]),'b'
os.makedirs(f'{targetdir}/flow4',exist_ok=True)
os.makedirs(f'{targetdir}/mask',exist_ok=True)
...
for bf in 'bf':
    ...
    for i, f in enumerate(sorted(glob.glob(f'{targetdir}/{bf}*.jpg'))[:frame + 1]):
        ...
        newmask = maskoutcontainer(image, False, maskfile=f'{targetdir}/mask/{basef}')
        if lastimage is not None:
            ...
            if 1:
                ...
                for k2, (k1, age) in linktoorigin.items():
                    ...
                    x2.append(k2[0])
                    y2.append(k2[1])
                    mind = min(mind, y2[-1] - y1[-1])
                    maxd = max(maxd, y2[-1] - y1[-1])
                data = []
                ...
                plt.xlim(0, 1280)
                plt.ylim(720, 0)
                plt.text(0, 100, mind, fontsize=16, fontweight='bold', color='red')
                plt.text(0, 200, maxd, fontsize=16, fontweight='bold', color='red')
                plt.savefig(f'{targetdir}/flow4/{basef}')
            if 0:
                plt.cla()
                ax = plt.gca()
                plt.imshow(image)
                ...
                plt.savefig(f'{targetdir}/lastflow/{basef}')
            # print 'done'
