...
def bsmatcher(image0,image1,newmask0,newmask1,draw=False,sift0=None):
    ...
    for match in list(matches[i][0] for i in np.argwhere(np.array(matchesMask))[:,0]):
        ...
        matched1[tuple(ckp1[match.queryIdx])]=1
        matched2[tuple(ckp2[match.trainIdx])]=1
    return out,ckp2,(kp2,des2),matched1,matched2
...
outdir=os.path.dirname(prefix).replace('/raw',f'/flowtest28/{origcamname}')
#outdir=sys.argv[2]
...
maxthresh = dict(txxx=20, cnxx=9999)
...
if 1:
    for f,t in zip(fs,ts):
        ...
        if lastimage[camname] is not None:
            out, kpxy, lastsift[camname],matched1,matched2 = bsmatcher(lastimage[camname], image[camname], lastnewmask[camname], newmask[camname], sift0=lastsift[camname])
            for x in out:
                p2,p3=x                                
                cv2.arrowedLine(image_cpy2[camname],(p2[0],p2[1]),(p3[0],p3[1]),(0,0,255),2)            
            unique1=set(p for p,v in matched1.items() if v==0)
            unique2=set(p for p,v in matched2.items() if v==0)
            for p in unique1:
                p=int(p[0]),int(p[1])
                cv2.arrowedLine(image_cpy2[camname],(p[0]-1,p[1]-1),(p[0]-1,p[1]-1),(255,0,0),2)
            for p in unique2:
                p=int(p[0]),int(p[1])
                cv2.arrowedLine(image_cpy2[camname],(p[0]+1,p[1]+1),(p[0]+1,p[1]+1),(0,255,0),2)

            print(camname,2,time.time()-T0)
            out = list((tuple(x[0]),tuple(x[1]),dist(x[0],x[1])) for x in out)
            #filter out sudden down movement while keeping cases as may correspond to moveoff
            outmoving = list((x[0], x[1]) for x in out if (x[2]<=distcutoff and not(x[2]>distcutoff_down1 and x[1][1]-x[0][1]>distcutoff_down2)) and x[2]>distcutoff_stationarydist and fgmask[camname][int(x[1][1]),int(x[1][0])]>0 and fgmask[camname][int(x[0][1]),int(x[0][0])]>0)
            outstationary = list((x[0],x[1]) for x in out if x[2]<=distcutoff_stationarydist)
            ...
            #keepalive all arrows ending within distcutoff_withindist
            for x in outstationary:
                p2,p3=x
                cv2.arrowedLine(image_cpy2[camname],(p2[0],p2[1]),(p3[0],p3[1]),(255,255,0),2)
                tmp1b = withindist(p2, tmp1, distcutoff_withindist)
                for p1,d in tmp1b:
                    p1=tuple(p1)
                    p0,age=lastlinktoorigin[camname][tuple(p1)]
                    if p1 in linktoorigin[camname]:
                        if linktoorigin[camname][p1][1]<=age+1:
                            linktoorigin[camname][p1] = (p0, age + 1)
                    else:
                        linktoorigin[camname][p1] = (p0, age + 1)
            #update linktoorigin for moving arrows
            for x in outmoving:
                p2,p3=x
                cv2.arrowedLine(image_cpy2[camname],(p2[0],p2[1]),(p3[0],p3[1]),(255,255,0),2)
                tmp1b = withindist(p2, tmp1, distcutoff_withindist)
                if len(tmp1b) > 0:
                    ...
                    (p0,age),d,p1 = max(list((lastlinktoorigin[camname][tuple(p)], d,tuple(p)) for p, d in tmp1b), key=lambda x: (x[0][1], -x[1]))
                    if p3 in linktoorigin[camname]:
                        if linktoorigin[camname][p3][1]<=age+1:
                            linktoorigin[camname][p3] = (p0, age + 1)
                    else:
                        linktoorigin[camname][p3] = (p0, age + 1)
                else:
                    #lastlink defaults to current point whose age is 0
                    linktoorigin[camname][p3] = (p2, 1)
        ...
        cv2.imwrite(f'{outdir}/{camname}_{t:06d}b.jpg',image_cpy[camname])
        cv2.imwrite(f'{outdir}/{camname}_{t:06d}a.jpg',image_cpy2[camname])
