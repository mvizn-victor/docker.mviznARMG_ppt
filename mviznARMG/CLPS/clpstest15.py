...
outdir=os.path.dirname(prefix).replace('/raw',f'/flowtest15/{origcamname}')
#outdir=sys.argv[2]
...
if 1:
    for f,t in zip(fs,ts):
        ...
        fgmask[camname] = cv2.dilate(fgmask[camname],kernel,iterations = 1)
        ...
        if lastimage[camname] is not None:
            ...
            out = list((tuple(x[0]), tuple(x[1])) for x in out if dist(x[1], x[0]) <= distcutoff and fgmask[camname][int(x[1][1]),int(x[1][0])]>0)
            print(out)
