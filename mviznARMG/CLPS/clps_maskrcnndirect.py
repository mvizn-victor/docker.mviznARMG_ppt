...
minthresh = dict(txxx=-55, cnxx=-30)
maxthresh = dict(txxx=40, cnxx=9999)
print(time.time()-Tstart,file=open('/tmp/clpsloadtime.txt','a'))
...
while True:
    while True:
        ...
        if plc.TLOCK and lastland and not plc.LAND:
            ...
            printandlog('',file=makedirsopen(f'{PHOTOLOGDIRNAME}/{DATE}_{TIME}_liftstart.txt','w'))
        if liftstart:break
        print('WAITING FOR TLOCK and (LAND go OFF)')
        for camname in camnames:
    ...
    while True:
        ...
        plc=readplc()
        if plc.TLOCK and plc.getEstHoistPos()>6000 or time.time()-liftstart>30:
            break
        for camname in camnames:
            if camname.startswith('t'): camtype = 'txxx'
            if camname.startswith('cn'): camtype = 'cnxx'
            image[camname]=cam[camname][:,:,::-1].copy()
            ...
            T=time.time()
            if saveimages:
                makedirsimwrite(f'{PHOTOLOGDIRNAME}/raw/{camname}_{int((T - liftstart) * 100):06d}.jpg', image[camname][:,:,::-1])
