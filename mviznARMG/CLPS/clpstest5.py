...
import time
model.detect([np.zeros((3,3,3),dtype=np.uint8)], verbose=0)
if 0:
    plc=readplc()
    SIDE=plc.SIDE
    SIZE=plc.size
...
for targetdir in targetdirs:
    os.makedirs(f'{targetdir}/flow4d',exist_ok=True)
    os.makedirs(f'{targetdir}/maskb',exist_ok=True)
    if len(glob.glob(f'{targetdir}/flow4d/*.jpg'))>0:
        continue
    for bf in 'bf':
        lastimage = None
        linktoorigin = dict()
        ...
        for i, f in enumerate(sorted(glob.glob(f'{targetdir}/{bf}*.jpg'))[:frame + 1]):
            ...
            image = cv2.imread(f)[:, :, ::-1].copy()
            image_cpy = image[:,:,::-1].copy()
            newmask = maskoutcontainer(image, False)#, maskfile=f'{targetdir}/maskb/{basef}')
            print("T1:", time.time() - start)
            if lastimage is not None:
                lastlastsift=lastsift
                out, kpxy, lastsift = bsmatcher(lastimage, image, lastnewmask, newmask,sift0=lastsift)            
                print("T2:", time.time() - start)
                out = list((tuple(x[0]), tuple(x[1])) for x in out if dist(x[1], x[0]) < 50)
                linktoorigin = dict()
                tmp1 = list(lastlinktoorigin.keys())
                for x in out:
                    tmp1b = withindist(x[0], tmp1, 10)
                    if len(tmp1b) > 0:
                        tmp2 = max(list((lastlinktoorigin[tuple(p)], d) for p, d in tmp1b), key=lambda x: (x[0][1], -x[1]))[0]
                    else:
                        tmp2 = (x[0], 0)
                    if x[1] not in linktoorigin or linktoorigin[x[1]][1] <= tmp2[1] + 1:
                        linktoorigin[x[1]] = (tmp2[0], tmp2[1] + 1)
                ...
                y2=[]
                ds=defaultdict(list)
                for k2, (k1, age) in linktoorigin.items():
                    x1.append(k1[0])
                    y1.append(k1[1])
                    x2.append(k2[0])
                    y2.append(k2[1])
                ...
                for i in range(3):
                    ...
                    if i<len(_3smallest):
                        cv2.putText(image_cpy, '%.2f'%_3smallest[i], (0, 100 + i * 100), font, 1, (0, 0, 255), 2, cv2.LINE_AA)
                    if 2-i<len(_3largest):
                        cv2.putText(image_cpy, '%.2f' % _3largest[2-i], (0, 400 + i * 100), font, 1, (0, 255,0), 2, cv2.LINE_AA)
                for _x1, _y1, _x2, _y2 in zip(x1, y1, x2, y2):
                    if _y2-_y1<=-55:
                        ...
                        color=(255,0,0)
                    cv2.arrowedLine(image_cpy,(_x1,_y1),(_x2,_y2),color,2)

            cv2.imwrite(f'{targetdir}/flow4d/{basef}',image_cpy)
            lastlinktoorigin = linktoorigin
            lastimage = image
            lastnewmask = newmask
