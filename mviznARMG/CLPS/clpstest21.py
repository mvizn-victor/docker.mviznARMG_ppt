...
outdir=os.path.dirname(prefix).replace('/raw',f'/flowtest21_{sys.argv[2]}/{origcamname}')
#outdir=sys.argv[2]
...
fgbg=dict()
fgmask=dict()
kernelbig = np.ones((21,21),np.uint8)
...
if 1:
    for f,t in zip(fs,ts):
        ...
        distcutoff=100
        distcutoff_stationarydist=5
        distcutoff_withindist=int(sys.argv[2])
