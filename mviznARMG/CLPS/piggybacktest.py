...
###COMMON###
imshow=dict()
os.system('rm /dev/shm/*_piggyback')
for camname in camnames:
    imshow[camname]=createShm(f'{camname}_piggyback')
...
if deploy:
    ...
    PHOTOLOGDIRNAME=f'/opt/captures/piggyback/photologs/{JOBDATE}/{JOBTIME}'
    LOGFILENAME=f'/opt/captures/piggyback/logs/{JOBDATE}.txt'
else:
    JOBDATE=Tjobstart.strftime("%Y-%m-%d")
    JOBTIME=Tjobstart.strftime("%H-%M-%S")
    PHOTOLOGDIRNAME=f'/opt/capturestest/piggyback/photologs/{JOBDATE}/{JOBTIME}'
    LOGFILENAME=f'/opt/capturestest/piggyback/logs/{JOBDATE}.txt'


mcrw.raw_write('piggyback_liftdetected',0)
if 0:
...
def clpscam(camname):
    ...
    while True:
        break
        mcrw.raw_write('piggyback_active',time.time())
        plc=readplc()
    ...
    while True:
        ...
        if 0:
            plc=readplc()
            if plc.TLOCK and plc.getEstHoistPos()>10000 or time.time()-liftstart>30 or plc.speed<0:
                break
        ...
        newmask[camname] = fgmask[camname]<0
        ...
        if camtype == 'txxx' and (mind <= minthresh[camtype] or largestdownclustersize>=3):
            liftdetected=1
            if mcrw.raw_read('piggyback_liftdetected',0)==0:
                mcrw.raw_write('piggyback_liftdetected',time.time())
            cornerstriggered=set()
            if camname.endswith('f'):
                cornerstriggered.update([2,3])
                mcrw.raw_write('piggyback_corner2',time.time())
                mcrw.raw_write('piggyback_corner3',time.time())
            if camname.endswith('b'):
                cornerstriggered.update([1,4])
                mcrw.raw_write('piggyback_corner1',time.time())
                mcrw.raw_write('piggyback_corner4',time.time())
            ...
            #sendjson('piggyback',camname.upper(),NOW,plc,f'{PHOTOLOGDIRNAME}/{DATE}_{TIME}_{camname}.jpg')
        if 0 and camtype == 'txxx' and (maxdx >= maxthreshx[camtype] or mindx <= minthreshx[camtype]):
