...
#fj13:clpstest35.py
#DETECT MOVEMENT OF TWISTLOCK, BARRIER/PLATE
#fj31:
#log landedimage
#maxslide trigger algo2lift
#ignore maxslide if there are twin20 twistlock/barrier
#gd20:
#remove condition disable after 30s
#gd21:
#no container detected, disable algo2lift
#moveoff => no algo2lift
#gf20
#fixed moveoff => no algo2lift
#gg03
#write algo2lift thresh beside algo2lift trigger
#gg10
#clps camcheck bit
#gj25
#CLPS topright hoistheight
#ha03
# hoistheight from plc and time%1000
#ha10
# hoistheight on first frame
#hd16
# algo1lift down require algo2lift down at least 5
# reverse also consider moveoff
#he13
# algo2lift use bottom of detection
#hf14
# use yc for down and y2 for up
def print(*x,**kw):
...
YELLOW=(0,255,255)
WHITE=(255,255,255)
def addtobin(bins,xc,y2,framei):
    found=0
    for k in bins:
        xmin,xmax=k
        if xmin-75<=xc and xc<=xmax+75:
            found=1
            xmin=min(xc,xmin)
            xmax=max(xc,xmax)
            tmp=bins[k]
            if framei-tmp[3]<=3:
                ...
                k=(xmin,xmax)
                bins[k]=tmp
                bins[k][0]=min(bins[k][0],y2)
                bins[k][1]=max(bins[k][1],y2)
                bins[k][3]=framei
            break
    if not found:
        k=(xc,xc)
        bins[k]=[y2,y2,y2,framei]
    return k,bins[k]
class Camstate:
    def __init__(self):
        self.bins=dict()
        self.binsyc=dict()
        self.hasclosemultiple=0
    def addtobinyc(self,xc,yc,framei):
        addtobin(self.binsyc,xc,yc,framei)
    def addtobin(self,xc,y2,framei):
        addtobin(self.bins,xc,y2,framei)
    def drawbins(self,im):
        for k in self.bins:
            xmin,xmax=k
            xc=(xmin+xmax)//2
            ymin=self.bins[k][0]
            ymax=self.bins[k][1]
            yfirst=self.bins[k][2]
            RED=(0,0,255)
            GREEN=(0,255,0)

            cv2.arrowedLine(im,(xc,yfirst),(xc,ymin),RED,2)
            putText(im,f'{yfirst-ymin}',(xc,ymin),color=RED,shadow=1)
            #cv2.arrowedLine(im,(xc,yfirst),(xc,ymax),GREEN,2)
            #putText(im,f'{ymax-yfirst}',(xc,ymax),color=GREEN,shadow=1)
            cv2.line(im,(xmin,yfirst),(xmax,yfirst),BLUE,2)
            putText(im,f'{xmax-xmin}',(xmax,yfirst),color=BLUE,shadow=1)
        for k in self.binsyc:
            xmin,xmax=k
            xc=(xmin+xmax)//2+2
            ymin=self.binsyc[k][0]
            ymax=self.binsyc[k][1]
            yfirst=self.binsyc[k][2]
            RED=(0,0,255)
            GREEN=(0,255,0)
    ...
    def get_maxup_maxdown(self):
        ...
        for k in self.binsyc:
            ...
            ymax=self.binsyc[k][1]
            yfirst=self.binsyc[k][2]
            maxdown=max(ymax-yfirst,maxdown)
        return maxup,maxdown
    def get_maxslide(self):
        maxslide=0
        for k in self.bins:
            xmin,xmax=k
            maxslide=max(maxslide,xmax-xmin)
        return maxslide
...
algo2liftupthresh=90
algo2liftdownthresh=25
algo2liftsidethresh=50
...
def clpscam(camname):
    if not deploy:
        ...
        plc=G()
        plc.HoistPos=0
        plc.getEstHoistPos=lambda:0
        Treadplc=0
    font = cv2.FONT_HERSHEY_SIMPLEX
    fgbg[camname] = cv2.createBackgroundSubtractorMOG2()
    camstate[camname] = Camstate()
    camstate[camname].framei=0
    liftstart=0
    liftdetected=0
    algo2lift=0
    lastland=0
    ...
    if deploy:
        ...
        while True:
            ...
            plc=readplc()
            Treadplc=time.time()
            if plc.TLOCK and plc.speed>0 and time.time()-landedimageT>0.4:
                landedimageT=time.time()
                makedirsimwrite(f'{PHOTOLOGDIRNAME}/landedimage/{camname}_0_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S-%f")}.jpg',cam[camname])
    ...
    cum_liftdown=0
    while True:
        if deploy:makedirsimwrite(f'{PHOTOLOGDIRNAME}/landedimage/{camname}_1_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S-%f")}.jpg', cam[camname])
        mcrw.raw_write('clps_active',time.time())
        NOW=datetime.now()
        ...
        #FIX:added for update gg10
        camname2=camname[:1]+camname[-1:]
        if deploy:
            plc=readplc()
            Treadplc=time.time()
            #if plc.TLOCK and plc.getEstHoistPos()>6000 or time.time()-liftstart>30 or plc.speed<0:
            if plc.TLOCK and plc.getEstHoistPos()>6000 or plc.speed<0:
                mcrw.raw_write('clps_processing',0)
                break
            print(datetime.now(),"bytes78",bin(plc.data[78]))  
            CLPS_StopProcessing = not plc.CLPS_Enable #repurposed
            if CLPS_StopProcessing:
                mcrw.raw_write('clps_processing',0)
                mcrw.raw_write('clps_active',0)
                time.sleep(0.25)
                continue
            else:
                mcrw.raw_write('clps_processing',time.time())
        ...
        camstate[camname].framei+=1
        list_res=procimage('clps_yolo',image[camname])

        #FIX: added for update gg10
        if len(list_res)>0:
            mcrw.raw_write(f'tcds_{camname2}',time.time())
        else:
            mcrw.raw_write(f'tcds_{camname2}',0)
        list_xc=[]
        for res in list_res:
            name = res[0]
            if name not in ['b','t']:continue
            prob = res[1]
            xc = int(res[2][0])
            list_xc.append(xc)
            yc = int(res[2][1])
            ...
            x2 = xc + boxw
            y2 = yc + boxh
            cv2.rectangle(image_cpy[camname],(x1,y1),(x2,y2),color=RED,thickness=2)
            #camstate[camname].addtobin(xc,yc,camstate[camname].framei)
            #he13
            camstate[camname].addtobin(xc,y2,camstate[camname].framei)
            camstate[camname].addtobinyc(xc,yc,camstate[camname].framei)
        list_xc.sort()
        if len(list_xc)>0:
            for xc1,xc2 in zip(list_xc[:-1],list_xc[1:]):
                if xc2-xc1<=200:
                    camstate[camname].hasclosemultiple=1
        camstate[camname].drawbins(image_cpy[camname])
        maxup,maxdown=camstate[camname].get_maxup_maxdown()
        maxslide=camstate[camname].get_maxslide()
        hasclosemultiple=camstate[camname].hasclosemultiple
        #gd21
        newmask[camname]=maskoutcontainer(image[camname], False)
        #gd21
        if np.sum(newmask[camname]==0)>0:
            hascontainer=1
        else:
            hascontainer=0
        ...
        if lastimage[camname] is not None:
            ...
            cv2.putText(image_cpy[camname], f'algo2lift:{algo2lift>0} |up:{maxup}/{algo2liftupthresh}|down:{maxdown}/{algo2liftdownthresh}|side:{maxslide}/{algo2liftsidethresh}', (0, 400 + 3 * 100), font, 1, (0, 255,255), 2, cv2.LINE_AA)
            ...
            print(camname,2,time.time()-T0)
        #gf20
        #if algo2lift==0 and ( (maxup>=90 or maxdown>=25) or (maxslide>=50 and not hasclosemultiple) ):
        cv2.putText(image_cpy[camname], f'HGTest:{int(plc.getEstHoistPos()):7d}',(3*1280//4,100),font,1,(0,0,255), 2, cv2.LINE_AA)
        #ha03
        cv2.putText(image_cpy[camname], f'HGTplc:{int(plc.HoistPos):7d}',(3*1280//4,200),font,1,(0,0,255), 2, cv2.LINE_AA)
        cv2.putText(image_cpy[camname], f'TIME:{Treadplc%1000:7.2f}',(3*1280//4,300),font,1,(0,0,255), 2, cv2.LINE_AA)
        if algo2lift==0 and not cum_moveoff and ( (maxup>=algo2liftupthresh or maxdown>=algo2liftdownthresh) or (maxslide>=algo2liftsidethresh and not hasclosemultiple) ) and hascontainer:
            algo2lift=1
        elif algo2lift==1:
            algo2lift=2
        ...
        if liftup or (liftdown and (len(list_xc)==0 or maxdown>5)) or algo2lift==1:
            liftdetected=1
