...
#fh10:
#corneryolo then extract image underneath until the top of corner in first frame
def print(*x,**kw):
...
def addtobin(bins,xc,y0,y1):    
    found=0
    for k in bins:
        xmin,xmax=k
        if xmin-50<=xc and xc<=xmax+50:
            xmin=min(xc,xmin)
            xmax=max(xc,xmax)
            tmp=bins[k]
            del bins[k]
            k=(xmin,xmax)
            bins[k]=tmp
            bins[k][0]=min(bins[k][0],y0)
            bins[k][1]=max(bins[k][1],y1)
            found=1
            break
    if not found:
        k=(xc,xc)
...
def clpscam(camname):
    ...
    while True:
        ...
        res=procimage('tcdstx_yolo',image[camname])
        print('tcdstx_yolo',res)
        def yolores_old_to_new(resold):
            classes=[]
            scores=[]
            boxes=[]
            for result in resold:
                label=result[0]
        ...
        labels,scores,rects=yolores_old_to_new(res)
        for label,(recti,rect) in zip(labels,enumerate(rects)):
            xc=rect[0]+rect[2]//2
            y0=rect[1]
            y1=rect[1]+rect[3]
            (xmin,xmax),(ymin,ymax,count)=addtobin(bins,xc,y0,min(y0+20 if label=='fp' else y0+40,y1-10))
            record=0
            if label=='fp':
                if ymax-ymin>80:
                    record=1
            ...
            if label=='np':
                if ymax-ymin>120:
                    ...
                    pass
            ymin=max(ymin,y0)
            ymax=min(ymin+200,ymax)
            COLOR=GREEN
            prob=0
            if record:
                #clpslift=clpsinception.infer(im[ymin:ymax,max(xmin-50,0):xmax+50])[0]
                clpslift,prob=procimage('clpslift_inception',image[camname][ymin:ymax,max(xmin-50,0):xmax+50])
                if clpslift=='p':
                    bins[(xmin,xmax)][2]+=1
                    if bins[(xmin,xmax)][2]>=detect_attach_thresh:
                        detect_attach=1
                    COLOR=RED
                else:
                    prob=1-prob
            cv2.rectangle(image_cpy[camname],(xmin-50,ymin,xmax+50-(xmin-50),ymax-ymin),COLOR,2)
            putText(image_cpy[camname],f'{ymax-ymin}/{bins[(xmin,xmax)][2]}/{prob:.2f}',(xmin-50,ymin),color=GREEN)
        ...
        if lastimage[camname] is not None:
            ...
            try:
                cum_detect_attach=int(cum_detect_attach|detect_attach)
            except:
                cum_detect_attach=int(detect_attach)
            ...
            cv2.putText(image_cpy[camname], f'detect_attach:{cum_detect_attach}/{detect_attach}', (0, 400 + 3 * 100), font, 1, RED, 2, cv2.LINE_AA)            
        ...
        if liftup or liftdown or detect_attach:
            liftdetected=1
