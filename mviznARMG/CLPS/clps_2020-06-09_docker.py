...
def clpscam(camname):
    ...
    while True:
        ...
        if liftstart:break
        if camname in ['cnlsbb','cnssbb']:print('WAITING FOR TLOCK and (LAND go OFF)')
...
if __name__ == '__main__':
    if deploy:
        lastT=time.time()
        while os.system('bash mrcnnserve.sh')!=0:
            print('mrcnnserve fail')
            print('Tmrcnnserve',time.time()-lastT)
            lastT=time.time()
        print('Tmrcnnserve',time.time()-lastT)
        if 0:
            while True:
