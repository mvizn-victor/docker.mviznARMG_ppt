...
def bsmatcher(image0,image1,newmask0,newmask1,draw=False,sift0=None):
    ...
    for i,(m,n) in enumerate(matches):
        if m.distance < 0.8*n.distance:
            ...
            x2,y2=int(x2),int(y2)
            if not newmask0[y1,x1] and not newmask1[y2,x2] and not(y2-y1>2 and dist((x1,y1),(x2,y2))>50):
                matchesMask[i]=[1,0]
...
outdir=os.path.dirname(prefix).replace('/raw',f'/flowtest25/{origcamname}')
#outdir=sys.argv[2]
