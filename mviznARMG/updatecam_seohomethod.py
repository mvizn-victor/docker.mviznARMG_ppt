...
os.makedirs('config/tcds',exist_ok=True)
os.makedirs('config/clps',exist_ok=True)
os.makedirs('config/pmnrs',exist_ok=True)
def getcamname(f):
...
try:
    oldcat=open('config/seoho_offsets/lastcat.txt').read()
except:
    oldcat=''
from datetime import datetime
if newcat==oldcat:
    print(datetime.now(),'not updating',file=open('config/seoho_offsets/log.txt','a'))
    quit()
else:
    from datetime import datetime
    print(datetime.now(),'updating',file=open('config/seoho_offsets/log.txt','a'))
    open('config/seoho_offsets/lastcat.txt','w').write(newcat)
try:
    pan,tilt=read_seoho_preset_folder(preset_folder)
    for f in glob.glob('config/seoho_offsets/pmnrs/*.txt')+glob.glob('config/seoho_offsets/clps/*.txt')+glob.glob('config/seoho_offsets/tcds/*.txt'):
        ...
        ptz=f'pan={p}\ntilt={t}\nzoom={z}'
        print(f,camname)
        fout=f.replace('config/seoho_offsets/','config/')
        assert 'seoho' not in fout
        print(fout)
        print(ptz)
        print(ptz,file=open(fout,'w'))
except Exception as e:
    print(datetime.now(),'fail',e,file=open('config/seoho_offsets/log.txt','a'))
    quit()
