...
if os.path.exists('config/usecv2dnn'):
    ...
    from tfstuff import hncdsinfer as infer
...
if usecv2dnn:
    ...
    hncdsinception=Inception('HNCDS/weights/HNCDS.pb')
    res=hncdsinception.infer(dummyimage)
    print('dummy',res)
    if 1:
        while True:
            try:
                f=min(glob.glob('/dev/shm/hncdsinception/*.jpg'))
                if 1:
                    print(0)
                    ...
                    print(4)
                    res=hncdsinception.infer(frame)
                    print(5)
                    print(res)
                    print(6)
