...
class MyVideoCapture:
    ...
    def getfps(self):
        if self.type == 'VideoCapture':
            return self.SELF.get(cv2.CAP_PROP_FPS)
        elif self.type == 'Image':
            return 1000
        else:
            return 1000
...
def readFrametoShm(videoinfo):
    camid=videoinfo[0]
    usevid=None
    if camid[-2:] in ['bf','bb','bc']:
        usevid='tcds'
        input_video='TCDS/sample/1.mp4'
    elif camid[-3:] in ['4xf','4xb']:
        usevid='hncds_side'
        input_video='HNCDS/sample/hncds_side.mp4'
    elif camid[-3:] in ['20f']:
        usevid='clps'
        input_video='CLPS/sample/clps_tl20f.mp4'
    elif camid[-3:] in ['20b']:
        usevid='clps'
        input_video='CLPS/sample/clps_tl20b.mp4'
    elif camid[:1] in ['p']:
        usevid='pmnrs'
        input_video='PMNRS/sample/1.jpg'
    elif camid[:2] == 'ov':
        usevid='hncds'
        input_video='HNCDS/sample/1.mp4'
    else:
        print('readframetoshm ERROR',videoinfo)
