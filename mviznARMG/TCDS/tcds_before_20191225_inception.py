...
if 0:
    #test gpu memory load
    print(YOLO.inferImg(np_image=np.zeros((3,3,3),dtype=np.uint8), thresh=0.2,
...
with tf.Session(config=tfconfig, graph=infer.graph) as sess:
    ...
    camtypes=dict()
    for camname in camnames:
        if camname.startswith('t'): camtypes[camname] = 't'
    ...
    while True:
        ...
        for camname in camnames:
            ...
            results = YOLO.inferImg(np_image=frame, thresh=0.2,
                                ...
                                weightPath="TCDS/weights/TCDSnp.weights",
                                metaPath="TCDS/weights/TCDSnp.data")
            if PAN_WAIT==1 and camname.endswith('bf'):
                if len(results)==0:
                    #assume always can detect corner if exists
                    #non detection imply panning in progress
                    print('PAN_WAIT','NONDETECTION')
                    PAN_WAIT=2                
                    camscores[camname] = defaultdict(list)
                    storedetection[camname] = dict()
                else:
                    for result in results:
                        xc = int(result[2][0])
                        yc = int(result[2][1])
                        if camname.endswith('bf') and isnewx(camscores[camname],xc):
                            print('PAN_WAIT','X SHIFTED')
                            #if new corner detected that is significantly different in position then also imply panning in progress
                            PAN_WAIT=2
                            camscores[camname] = defaultdict(list)
                            storedetection[camname] = dict()
                            break
            ...
            for x in camscores[camname]:
                ...
                if SIZE!=45 or SIZE==45 and camname.startswith('t'):
                    ...
                    if len(camscores[camname][x])>=3:                        
                        if all(score>=0.7 for score,p in camscores[camname][x][-3:]):
