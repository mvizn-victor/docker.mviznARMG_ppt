...
#gf07:
#tpos not in [0,11] skip
#gk22
#subyolo ytop < subimgheight/10 ignore
#ha31
...
import os
try:
    #run tcdstx instead if config/tcdstx is activated
    if os.path.exists('config/tcdstx'):
        print('running degraded tcds')
        import tcdstx
        sys.exit(0)
except:
    pass
...
if 1:
    ...
    lastheight=99999
    corners_responsible=dict()
    corners_detected=dict()
    for camname in camnames:
        camtype=camtypes[camname]
        corners_detected[camtype]=set()
        corners_responsible[camtype]=set()
        if SIZE==45 and camtype=='cf':
            corners_responsible[camtype].update(set(x for x in cornermap[SIZE,camname,1] if x<=4))
            corners_responsible[camtype].update(set(x for x in cornermap[SIZE,camname,2] if x<=4))
        else:
            corners_responsible[camtype].update(set(x for x in cornermap[SIZE,camname] if x<=4))
    while True:
                ...
                def proc1(phase=0):
                    ...
                    def proc2(x,corner):
                        ...
                        if x in storedetection[camname]:
                            _DATE, _TIME, _frame_cpy, _frame, _xc, _yc = storedetection[camname][x]
                            ...
                            (255, 0, 0), 2)
                                ...
                                makedirsimwrite(f'{PHOTOLOGRAWDIRNAME}/{_DATE}_{_TIME}_{camname}_{corner}.jpg', _frame)
                        ...
                        if lognegative:
                            if x in negstoredetection[camname]:
                                _DATE, _TIME, _frame_cpy, _frame, _xc, _yc = negstoredetection[camname][x]
                                if not negdetected[camname, corner]:
                                    negdetected[camname, corner] = True 
                                    cv2.putText(_frame_cpy, f'{corner}', (_xc, _yc), cv2.FONT_HERSHEY_SIMPLEX, 1,
                            (255, 0, 0), 2)
                                    makedirsimwrite(f'{PHOTOLOGDIRNAME}/x/{camname}_{corner}.jpg', _frame_cpy)
                                    corners_detected[camtype].add(corner)
                                    print('corner detected',camtype,corner)
                                    if corners_responsible[camtype]-corners_detected[camtype]==set():
            ...
            if PAN_WAIT>2:
                if camname.endswith('bf'):
                    ...
                    try:
                        ...
                        camatpos=0
                    if not camatpos and time.time()-lastcheckcamatpos>0.1:
                        camatpos=eval(camname).atpostxt(f'config/tcds/{camname}45-2.txt')
                        lastcheckcamatpos=time.time()
                    if camatpos and time.time()-lastcheckcamatpos>0.25:
                        ...
                        continue
            #print(camname,tmp,camrawlastT[camname])
            if tmp == camrawlastT[camname]: 
                print(camname,'stale')
                continue  # skip stale images
            camrawlastT[camname] = tmp
            results = YOLO.inferImg(np_image=frame, thresh=0.2,
                                configPath="TCDS/weights/TCDSnp.cfg",
                                weightPath="TCDS/weights/TCDSnp.weights",
                                metaPath="TCDS/weights/TCDSnp.data")
            #extra logging 2022-07-06
            if len(results)>0 and plc.TLOCK and plc.speed>0:
                ...
                if SIZE!=45 or SIZE==45 and camname.startswith('t'):
                    ...
                    if hit and (max(hitsy)-min(hitsy)>hitsy_diffthresh):
                        ...
                        if not detected[camname,corner]:
                            ...
                            makedirsimwrite(f'{PHOTOLOGRAWDIRNAME}/{DATE}_{TIME}_{camname}_{corner}.jpg',frame)
                    ...
                    if hit and (camtype!='cf' and max(hitsy)-min(hitsy)>hitsy_diffthresh or camtype=='cf'):
                        if x not in storedetection[camname]:
                            storedetection[camname][x]=[DATE,TIME,frame_cpy,frame,xc,yc]
                    if lognegative:
